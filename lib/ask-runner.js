 ...
 *
 * WAS llama-3.3-70b-versatile UNTIL GROQ REMOVED IT. Measured 3 September 2026:
 * the model list no longer carried it, every question on the live site came back
 * `Groq 404 model_not_found`, and the site had been answering nothing for however
 * long that had been true — the decommission email named only the 8B model, and
 * the 70B followed it without one. openai/gpt-oss-120b is Groq's own replacement
 * tier and routes the real tool catalogue correctly (verified: "hows nvda doin"
 * → lookup_token, temperature 0, ~1.2s).
 *
 * THE ROUTING WAS TUNED AGAINST THE OLD MODEL and has not been re-benched against
 * this one. The tool descriptions and the temperature split in lib/ask-loop.js
 * were measured over 103 questions × 3 repetitions on llama-3.3-70b; a different
 * model reroutes in ways nothing offline catches. Run `npm run route:bench` and
 * compare before trusting the numbers. A working model beats a perfectly tuned
 * dead one, which is the only reason this ships without that bench first.
 */
