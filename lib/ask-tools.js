...
export const TOOL_SCHEMAS = Object.freeze([
  ...
  },
  {
    ...
    function: {
      ...
      description:
        ...
        "A QUESTION THAT NAMES A LENGTH OF TIME GETS thresholdDays, AND ITS ANSWER IS THE COUNT. \"how many have held more than 3 days\" is thresholdDays 3, \"over a week\" is 7. The result then carries an \"overThreshold\" block with over / under / undetermined out of ONE stated denominator, and a finished \"reading\" — quote that. Do NOT answer a threshold question from the median or the range: measured live, asked how many had held more than three days, the range \"at least 0.1 to 39.4 days\" was offered as the answer, which is a different question wearing a number. " +
        "UNDETERMINED IS NOT A NO, and this is the one that inverts if you get it wrong. A row reading \"at least 12 days\" against a 3-day mark is a definite YES — the floor already clears it. A row reading \"at least 2 days\" against the same mark settles NOTHING, because its history ran past what was read and the true age is larger by an unknown amount. Those land in undetermined, and reporting them as holders who did not qualify would understate conviction on the strength of us having stopped looking. " +
        "AGE IS NOT CONTINUOUS HOLDING, AND \"WITHOUT SELLING\" IS NOT SOMETHING THIS MEASURES. Every figure here is the age of an address's OLDEST readable transfer of the token. An address that bought a month ago, sold the lot, and bought back yesterday still reads as a month. So never write \"held N days without selling\", never call these addresses diamond hands on this evidence, and if the reader asked specifically about selling, say that this shows when they first bought and that whether ONE named wallet has ever sold is trace_wallet, which needs that wallet and a token. " +
      ...
      parameters: {
        ...
        properties: {
          ...
          thresholdDays: {
            type: "number",
            description:
              "OPTIONAL, and only when the question names a length of time: how many days to count against. \"more than 3 days\" is 3, \"over a week\" is 7, \"more than a month\" is 30. Leave it out entirely when the question is just how long they have held — a threshold nobody asked for answers a question nobody asked.",
          },
        },
        required: ["query"],
        additionalProperties: false,
  ...
  },
  {
    ...
    function: {
      name: "trace_funds",
      description:
        "FOLLOW ONE TOKEN OUT OF ONE WALLET, HOP BY HOP, and say where each part of it stopped. Use it when the question is where funds ENDED UP rather than who they went to, in any phrasing, casing or language: \"trace these funds\", \"where did my tokens end up\", \"follow the money from 0x…\", \"did it reach an exchange\", \"sigue el rastro\". " +
        "IT NEEDS A WALLET AND A TOKEN. Use wallet_flows first when no token is named — it shows what left and in which tokens, and its answer tells you which token is worth tracing. " +
        "THE FOUR BUCKETS NEVER MERGE, AND THAT IS THE WHOLE POINT. \"endedTerminal\" is value that is provably not moving again — burned, sent to the token's own contract, or sitting with an address whose WHOLE history was read. \"endedInAPool\" was TRADED: it did not stop, it became another token, and this does not follow it — never describe that as funds recovered, as a destination, or as a completed trace. \"returnedToOrigin\" came back. \"trailOpen\" is value whose next move is UNKNOWN, because the receiving address had more history than could be read or the hop budget ran out first. Open is not held, and it is not lost. " +
        "QUOTE \"reading\", WHICH ALREADY STATES ALL OF THIS IN THE RIGHT PROPORTIONS. Each row's \"meaning\" carries the sentence for that ending; use it rather than writing your own, especially for \"not_read\", which means nobody looked and NOT that the address is holding. " +
        "\"reconciles\" false means the buckets do not add up to what left, so say the figures are indicative and quote \"unaccounted\". \"branchesNotFollowed\" counts destinations the trace never opened, largest first, so a big number there means this is a partial picture. \"poolsKnown\" false means a trade may appear here as an ordinary transfer. " +
        "NEVER NAME A COMPANY, AN EXCHANGE OR A PERSON AS A DESTINATION, and never say two addresses belong to the same owner. Report where value went and what each ending means, and let the reader decide who to contact.",
      parameters: {
        type: "object",
        properties: {
          address: {
            type: "string",
            description: "The wallet the funds left — 0x followed by exactly 40 hex characters.",
          },
          token: {
            type: "string",
            description: "The token to follow: a ticker (\"NVDA\"), a company name, or a 0x contract address.",
          },
        },
        required: ["address", "token"],
        additionalProperties: false,
  ...
  },
  {
    ...
    function: {
      name: "wallet_flows",
      description:
        "WHERE ONE WALLET'S VALUE WENT AND CAME FROM, ACROSS EVERY TOKEN, newest first, with who was on the other side. This is the investigation lookup, in any phrasing, casing or language: \"where did the funds go\", \"show me the last transfers out\", \"my wallet was drained, what happened\", \"who did this address send to\", \"trace this wallet\", \"a donde fueron los fondos\". " +
        "USE IT INSTEAD OF trace_wallet WHEN NO TOKEN IS NAMED. trace_wallet needs a token and reads only that one, which is the wrong shape for a drain — funds leave in whatever they were last swapped into, so a single-token view shows an emptied balance and none of the exits. wallet_portfolio is what it holds NOW; this is what moved. " +
        "COUNTERPARTIES CARRY ROLES AND LABELS, AND THE TWO ARE DIFFERENT KINDS OF FACT. \"role\" is established by lookup — a burn address, or one of Robinhood's issuer-verified equity contracts — and may be stated plainly. \"labelReading\" is the EXPLORER'S label for that address and is already worded as such: quote it as given, never harden it into \"this is X\", and never call an address an exchange, a bridge or a company on your own account. " +
        "NEVER SAY TWO ADDRESSES BELONG TO THE SAME PERSON. Nothing here establishes that, and no arrangement of transfers can: a shared counterparty, a round trip and matching timing are produced by exchanges, bridges, routers and distributors every day. Report who dealt with whom and let the reader conclude. " +
        "\"shape\" DESCRIBES THE WALLET YOU ASKED ABOUT AND NOTHING ELSE, and it is null whenever too little was read to say. When it is set, quote \"shapeReading\" AS WRITTEN — it already names the innocent explanations that produce the same shape, and dropping them turns a measurement into an allegation. Never apply a shape to a COUNTERPARTY: none is computed for them, because that would mean reading each one's own activity, and a shape guessed from how an address looks in this one feed is exactly the unmeasured claim this refuses. " +
        "NO VALUE IS COMPUTED ANYWHERE IN THIS RESULT. QUOTE EACH COUNTERPARTY'S \"summary\" AND DO NOT BUILD YOUR OWN FROM ITS FIELDS. That string already reads \"12 transfers (12 in, 0 out) across 2 tokens — counts, not amounts\", which is the whole truth about that row. Measured live three times, composing from the raw fields instead produced \"$0.15 sent\" and \"$2,449.26 received\" — money that exists nowhere in this result and that nothing here could compute, because no price is read in this lookup at all. Never attach a currency symbol to any figure from this tool, and if the reader asks what the flows were WORTH, say plainly that this counts movements rather than pricing them and offer wallet_portfolio for current value. Per-transfer amounts live in the table's amount column, already carry their token unit, and are never summed. " +
        "THE COUNTS ARE OVER WHAT WAS READ. \"historyComplete\" false means these are the most recent transfers and not all of them, so \"everything went to one address\" is a statement about the sample — quote \"outboundConcentration.reading\", which already carries its own denominator. " +
        "WHO SIGNED A TRANSFER OUT IS NOT AN ALARM. \"outboundSignedByThirdParty\" counts transfers that left in a transaction this address did not sign, and that is the ORDINARY case: routers, aggregators and smart accounts sign on the owner's behalf in most trades. Quote \"signerNotice\" as written, and never report it as a stolen approval or say an allowance is still live — no allowance is read here. \"outboundSignerUnknown\" is a count that could not be checked, which is not zero.",
      parameters: {
        type: "object",
        properties: {
          address: {
...
export function coerceHoldTimeQuery(args) {
  const target = coerceTokenTarget(args, "holder_hold_time");
  if (!target.ok) return target;
  /*
   * A THRESHOLD IS ONLY EVER OPTIONAL, AND A BAD ONE IS DROPPED RATHER THAN
   * DEFAULTED. Defaulting a malformed value would answer a question nobody asked
   * with a number that looks measured — "more than 30 days" when the reader said
   * "a while" — so anything that is not a finite, non-negative, sane number
   * simply means no threshold block, and the answer falls back to the median and
   * the range it always had.
   */
  const raw = args?.thresholdDays ?? args?.threshold_days ?? args?.days;
  const n = typeof raw === "string" ? Number(raw.trim()) : raw;
  const thresholdDays =
    typeof n === "number" && Number.isFinite(n) && n >= 0 && n <= 36_500 ? n : null;
  return { ...target, thresholdDays };
}
 ...
 * @returns {{ ok: true, value: string } | { ok: false, error: string }}
 */
export function coerceAddressArg(args, toolName = "lookup_wallet") {
  return coerceWalletAddress(args, toolName);
}
...
export async function dispatchTool(name, args, impls) {
  ...
  try {
    ...
    if (tool === "holder_hold_time") {
      const h = coerceHoldTimeQuery(args);
      if (!h.ok) return h;
      return await fns.holderHoldTime(h.value, h.thresholdDays === null ? {} : { thresholdDays: h.thresholdDays });
    }
    ...
    if (tool === "trace_funds") {
      const t = coerceTraceArgs(args, "trace_funds");
      if (!t.ok) return t;
      return await fns.traceFunds(t.address, t.token);
    }

    if (tool === "wallet_flows") {
      const a = coerceAddressArg(args, "wallet_flows");
      if (!a.ok) return a;
      return await fns.walletFlows(a.value);
    }
